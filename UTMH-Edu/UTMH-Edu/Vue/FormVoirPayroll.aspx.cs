...
{
    public partial class FormVoirPayroll : Page
    {
        ...
        private void ChargerPayroll()
        {
            ...
            try
            {
                ...
                // ✅ 12 mwa yo + LEFT JOIN payroll
                // ⚠️ Si non tab/kolòn yo diferan, modifye yo la a:
                string sql = @"
;WITH Mois AS (
...
SELECT
    ...
    CASE WHEN p.idPayroll IS NULL THEN 'NON FAIT' ELSE 'FAIT' END AS Statut,
    m.MoisNum
FROM Mois m
...
";
        ...
        {
            // ⚠️ Si table professeur a diferan non: modifye li
            using (var con = new SqlConnection(cs))
